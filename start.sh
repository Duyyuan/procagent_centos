...
if [ -z "$PID_httpsvr" ]
then
    nohup python27 ${workspace}/HttpServer.py ${workspace} > /tmp/httpsvr.log 2>&1 &
...
if [ -z "$PID_poll" ]
then
    python27 ${workspace}/PollManager.py start
    python27 ${workspace}/PollManager.py setintvl 6
    python27 ${workspace}/PollManager.py setpoll "['pollster.cpu.CPUUsagePollster','pollster.mem.MemInfoPollster','pollster.load.LoadStatPollster','pollster.disk.DiskUsagePollster','pollster.net.NetStatPollster','pollster.uptime.UptimePollster']"
